...
const EditGarraPremioCliente = (props) => {
    const { id } = useParams();
    ...
    const [garrapegada, setGarrapegada] = useState(null); // Estado para armazenar o valor da garra forte
    ...
    const fetchGarrapegada = () => {
        ...
        .then((res) => {
            setGarrapegada(res.data.garrapegada); // Atualize o estado com o valor da garra forte recebido
        })
        .catch((err) => {
    ...
    const onSave = () => {
        ...
        .then(res => {
            ...
            if (parseFloat(data.valor) === garrapegada) {
                ...
                setNotiMessage({
                    type: 'success',
                    message: `CONFIGURANDO GARRA PREMIO! AGUARDE O VALOR SER MODIFICADO PARA CONFIGURAR OUTRA FUNCAO!`
                });
            }
        })
            .catch(err => {
    ...
    useEffect(() => {
        fetchGarrapegada(); // Fetch da garra forte ao montar o componente
        const interval = setInterval(fetchGarrapegada, 10000); // Atualiza a cada 10 segundos
        return () => clearInterval(interval); // Limpa o intervalo ao desmontar o componente
    }, [id]); // Execute sempre que o ID mudar

    useEffect(() => {
        checkMatch(); // Verifique a correspondência ao montar o componente
    }, [data.valor, garrapegada]); // Execute sempre que o valor do campo de entrada ou a garra forte mudar
            ...
            <div className="AddMachine_container">
                <div className="AddMachine_header">
                    <div className="AddMachine_header_title">
                        GARRA PREMIO {maquinaInfos.nome}
                    </div>
                ...
                <div className="AddMachine_content">
                    ...
                    <div className="garrapegadaResult" style={{ fontSize: '24px' }}>
                    VALOR GARRA PREMIO: {garrapegada !== null ? garrapegada : "NULL..."}
                    </div>
...
export default EditGarraPremioCliente;
