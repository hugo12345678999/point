...
const EditGarraMediaCliente = (props) => {
    const { id } = useParams();
    ...
    const [garramedia, setGarramedia] = useState(null); // Estado para armazenar o valor da garra forte
    ...
    const fetchGarramedia = () => {
        ...
        .then((res) => {
            setGarramedia(res.data.garramedia); // Atualize o estado com o valor da garra forte recebido
        })
        .catch((err) => {
    ...
    const onSave = () => {
        ...
        .then(res => {
            ...
            if (parseFloat(data.valor) === garramedia) {
                ...
                setNotiMessage({
                    type: 'success',
                    message: `CONFIGURANDO GARRA MEDIA! AGUARDE O VALOR SER MODIFICADO PARA CONFIGURAR OUTRA FUNCAO!`
                });
            }
        })
            .catch(err => {
    ...
    useEffect(() => {
        fetchGarramedia(); // Fetch da garra forte ao montar o componente
        const interval = setInterval(fetchGarramedia, 10000); // Atualiza a cada 10 segundos
        return () => clearInterval(interval); // Limpa o intervalo ao desmontar o componente
    }, [id]); // Execute sempre que o ID mudar

    useEffect(() => {
        checkMatch(); // Verifique a correspondência ao montar o componente
    }, [data.valor, garramedia]); // Execute sempre que o valor do campo de entrada ou a garra forte mudar
        ...
        <>
            ...
            <div className="AddMachine_container">
                ...
                <div className="AddMachine_content">
                    ...
                    <div className="garramediaResult" style={{ fontSize: '24px' }}>
                    VALOR GARRA MEDIA: {garramedia !== null ? garramedia : "NULL..."}
                    </div>
...
export default EditGarraMediaCliente;
